...
class DarkMatterAnnihilationSpectralModel(SpectralModel):
    ...
    * `2011JCAP...03..051 <https://ui.adsabs.harvard.edu/abs/2011JCAP...03..051C>`_
    ...
    jfactor = Parameter("jfactor", 1, unit= u.Unit("GeV2 cm-5"))

    tag = ["DarkMatterAnnihilationSpectralModel", "dm-annihilation"]

    def __init__(self, mass, channel, sv=sv.quantity, jfactor=jfactor.value, z=0, k=2):
        self.k = k
        self.z = z
    ...
    def from_dict(cls, data):
        ...
        sv = [p["value"] for p in parameters if p["name"] == "sv"][0]
        return cls(sv=sv, **data)

class DarkMatterDecaySpectralModel(SpectralModel):
    r"""Dark matter Deact spectral model.
    ...
    mass : `~astropy.units.Quantity`
        Dark matter mass
    channel : str
        Decay channel for `~gammapy.astro.darkmatter.PrimaryFlux`
    tau : float
        Scale parameter for model fitting
    ...
    * `2011JCAP...03..051 <https://ui.adsabs.harvard.edu/abs/2011JCAP...03..051>`_
    ...
    TIME_DECAY = 1e26 * u.Unit("s")
    """Dark matter time decay"""
    ...
    dfactor = Parameter("dfactor", 1 * u.Unit("GeV cm-2"))
    """DFactor free parameter for model fitting"""
    
    def __init__(self, mass, channel, tau=tau.quantity, dfactor=dfactor.value, z=0, k=2):
        self.k = k
        self.z = z
        self.mass = mass
        self.channel = channel
        self.primary_flux = PrimaryFlux(mass, channel=self.channel).table_model
        super().__init__(tau=tau, dfactor=dfactor)

    def evaluate(self, energy, tau, dfactor):
        ...
        flux[energy > 2*self.mass] = 1e-50 * u.Unit("GeV-1 cm-2 s-1")

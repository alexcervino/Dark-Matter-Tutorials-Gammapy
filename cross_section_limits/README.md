...
This folder contains the core Python scripts used to derive upper limits on the Dark Matter (DM) annihilation cross-section ($\langle \sigma v \rangle$) for annihilation (limits_annihilation.py) or decay (limits_decay.py). 
...
**Important note:** For running these scripts you need to use the spectra.py file in this folder, since it is a duplicate of the spectra.py of Gammapy but with some modification madeby the authors of the KSP paper mentioned before. Changing the spectra.py file of Gammapy in your environment with the one in the folder would be enough.

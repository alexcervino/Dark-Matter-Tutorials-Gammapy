...
the 95% C.L. upper limits on the Dark Matter annihilation cross-section.
...
def getArgs():
   parser = argparse.ArgumentParser(
      description="Calculates upper limits for DM annihilation cross-section."
   )
   ...
   parser.add_argument('-b', '--basepath', default='.',
                        help='Base directory path containing the input materials (IRFs, Jfactors, etc.)')
...
class DMMapDataset(MapDataset):
    ...
    def jnuisance(self):
        lg_jfactor = np.log10(self.models.parameters["jfactor"].value)
        jobs = self.nuisance["jobs"].value
        lg_jobs = np.log10(jobs)
        sigma = self.nuisance["sigmaj"].value
        
        ee = np.power(lg_jfactor - lg_jobs, 2) / (2 * sigma * sigma)
...
# Helper Functions
# ==========================================
def jfactor_inter1d(path_j, name, profile):
    """Interpolates the J-factor based on the alpha value."""
    filename = os.path.join(path_j, profile, f'rvir_{name}_MCMC.dat.Jalphaint_cls.output')
    df_j = pd.read_csv(filename, sep=r'\s+', header=None, comment='#',
                       names=['alpha', 'jfactor', 'd1', 'd2', 'd3', 'd4', 'd5', 'd6', 'd7'])
    return interp1d(df_j.alpha.values, df_j.jfactor.values, kind="quadratic")

def setModel(name, mass, ch, j_factor, spatial_model, xsection):
    ...
    flux_model_fit = DarkMatterAnnihilationSpectralModel(mass=mass, channel=ch, jfactor=j_factor)
    ...
    models_fit.parameters["sv"].frozen = False
    models_fit.parameters["sv"].value = 1
    models_fit.parameters["sv"].min = 1e-6
    models_fit.parameters["jfactor"].min = 1e16 
...
if __name__ == "__main__":
    ...
    path_j = os.path.join(mainpath, "cross_section_limits", "clumpy", "Jfactors", "profile")
    path_irfs = os.path.join(mainpath, "cross_section_limits", "IRFs", "cta", "prod5-v0.1", "bcf")
    path_2d = os.path.join(mainpath, "cross_section_limits", "clumpy", "Jfactors", "2Dmaps")
    ...
    JFAC_sigma = df_input['jfac_sigma'].values
    ...
    for i, src in enumerate(dSphNames):
        j_val = jfactor_inter1d(path_j, name=src, profile=args.profile)(alpha[i]) * u.Unit("GeV2 cm-5")
        JFAC.append(j_val)
        
        jfactor_filename = os.path.join(path_2d, args.profile, f'annihil_{src}2D_FOVdiameter10.0deg_nside1024-JFACTOR-Jtot-image.fits')
        hdul.append(fits.open(jfactor_filename))
        spatial_model.append(TemplateSpatialModel.read(jfactor_filename))
    ...
    # Setup Output Table
    columns = ['profile', 'ch', 'mass', 'sigma_v']
    ...
    for run in range(args.nruns):  
        ...
        for ch in channels:
            for mass in masses:
                ...
                for i, src in enumerate(dSphNames):
                    dataset[i].models = setModel(f'dataset_{src}', mass, ch, JFAC[i], spatial_model[i], xsection)
                    dataset[i].nuisance = dict(j=JFAC[i], jobs=JFAC[i], sigmaj=JFAC_sigma[i] * u.Unit("GeV2 cm-5"), sigmatau=0.01)

                # Link sv parameter for stacking
                for i in range(len(dSphNames) - 1):
                    dataset[i+1].models[0].spectral_model.sv = dataset[0].models[0].spectral_model.sv
                ...
                try:
                    ...
                    datasets.models.parameters["sv"].frozen = True
                    fit.run(datasets=datasets)
                    lg_MLE_null = datasets.stat_sum()

                    # Fit 2: Unfreeze SV
                    datasets.models.parameters["sv"].frozen = False
                    fit.run(datasets=datasets)

                    # Fit 3: Unfreeze J-factor (Joint Fit)
                    for i, src in enumerate(dSphNames):
                        datasets.models[f'dataset_{src}'].parameters['jfactor'].frozen = False
                    ...
                    scale_min = datasets.models.parameters["sv"].value
                    scale_error = datasets.models.parameters["sv"].error  
                    ...
                    counter = 0
                    while True:
                        datasets.models.parameters['sv'].scan_n_values = 1
                        datasets.models.parameters['sv'].scan_min = scale_max
                        datasets.models.parameters['sv'].scan_max = scale_max
                        
                        ts_diff = np.max(fit.stat_profile(reoptimize=True, datasets=datasets, parameter='sv')['stat_scan'] - lg_MLE_null)
                    ...
                    datasets.models.parameters['sv'].scan_n_values = 100
                    datasets.models.parameters['sv'].scan_min = scale_min
                    datasets.models.parameters['sv'].scan_max = 2 * scale_max
                    
                    profile = fit.stat_profile(reoptimize=True, datasets=datasets, parameter='sv')
                    xvals = profile[f"dataset_{dSphNames[0]}.spectral.sv_scan"]
                    ...
                    sigma_v = (scale_found * xsection).value
                    print(f"Calculated Limit for {mass}: {sigma_v:.2e}")
                ...
                tmp_row.update({'profile': args.profile, 'ch': ch, 'mass': mass.value, 'sigma_v': sigma_v})
                
                final_table = pd.concat([final_table, pd.DataFrame([tmp_row])], ignore_index=True)

    # Clean and Save
    final_table['mass'] = final_table['mass'].astype(int)
    final_table['sigma_v'] = final_table['sigma_v'].astype(float)

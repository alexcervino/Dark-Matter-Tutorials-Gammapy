...
# 
# 
# In Gammapy, the profile likelihood is computed with the `Fit.stat_profile()` method: given a dataset with the model already fitted, `stat_profile` fixes its internal `scale` parameter (not our goal parameter, they are alled the same but they differ) to each value in a user-defined grid, re-optimizes all other free parameters at each point, and returns the corresponding `stat` (and hence $\Delta\text{TS}$) values, from which the desired confidence limit or interval can be read off.
# 
...
# We set ony 10 masses to make this tutorial quick, this can take several minutes, but you should cover much more.
